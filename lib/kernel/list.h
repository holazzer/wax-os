...
struct list {
	struct list_elem head;	// in this impl, head is empty
	struct list_elem tail;	// in this impl, tail remains the same and also always empty.
};
...
bool list_is_empty(struct list* plist);
